...
test("wires the 3:4 image editor, custom fonts, and draft export", async () => {
  ...
  assert.match(materialCenter, /用它创建内容/);
...
test("wires the persistent image library while retaining the Baidu cache service", async () => {
  ...
  assert.match(materialCenter, /小红书素材工作台/);
  assert.match(materialCenter, /上传成品图/);
  assert.match(materialCenter, /设置图位和标签/);
  assert.match(materialCenter, /编辑分组/);
  assert.match(materialCenter, /删除分组/);
  assert.match(materialCenter, /直接导出/);
  assert.match(materialCenter, /满足全部标签/);
  assert.match(materialCenter, /创建内容/);
  assert.match(materialCenter, /一键下载 ZIP/);
